    ...
    def search(self):
        ...
        for i in range(len(self.cbook.get_trie().query(search_term))):
            self.contacts_list.insert(
                i, self.cbook.get_search_result()[i].get("name"))
    ...
    def remove(self):
        if self.contacts_list.curselection():
            ...
            if self.has_searched:
                list_of_contacts = self.cbook.get_search_result()
            else:
                list_of_contacts = self.cbook.get_contacts()

            for key in self.contacts_list.curselection():
                self.name_to_remove = list_of_contacts[key].get("name")
    ...
    def update_list(self):
        ...
        if (len(self.cbook.get_contacts()) > 0):
            for i in range(len(self.cbook.get_trie().query(""))):
                self.contacts_list.insert(
                    i, self.cbook.get_contacts()[i].get("name"))
